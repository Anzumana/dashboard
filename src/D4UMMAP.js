...
class D4UMMAP extends Component {
  ...
  _renderLayers() {
    ...
    const layers = [];
    if(this.props.accessScoreBike){
      layers.push(
        new GeoJsonLayer({
          id: 'geojson',
          data: this.props.accessScoreBike,
          pickable: true,
          stroked: false,
          filled: true,
          extruded: true,
          ...
          getLineColor: d => {
            if(d.properties.Axs_score>= 1){
              return [249, 105, 14, 200];
            }
            if(d.properties.Axs_score > 0.75 &&  d.properties.Axs_score < 1){
              return [255, 203, 5, 200];
            }
            if(d.properties.Axs_score < 0.75 &&  d.properties.Axs_score >= 0.5){
              return [63, 195, 128, 200];
            }
            if(d.properties.Axs_score < 0.5 &&  d.properties.Axs_score >= 0.25){
              return [65, 131, 215, 200];
            }
            if(d.properties.Axs_score < 0.25 &&  d.properties.Axs_score >= 0){
