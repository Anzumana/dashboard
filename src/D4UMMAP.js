...
import DeckGL, {
  LineLayer, IconLayer, ScatterplotLayer, PathLayer, GeoJsonLayer,
} from 'deck.gl';
...
class D4UMMAP extends Component {
  ...
  calcPosition(val) {
    ...
    const coordinate = new Array(parseFloat(substrings[0]), parseFloat(substrings[1]));
    return coordinate;
  }
  ...
  componentWillReceiveProps(props) {
    ...
    try {
      const filteredEvents = props.events.filter((event) => ((Date.parse(event.start.replace('+01', 'Z'))) <= Date.parse(until)) && (Date.parse(event.start.replace('+01', 'Z')) >= Date.parse(since)));
      this.setState({
        filteredEvents,
        selectEvent: props.selectEvent,
      });
