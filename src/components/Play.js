...
export default (props) => {
  const next = () => {
    const since = new Date(props.filter.since);
    const until = new Date(props.filter.until);
  ...
  return (
    <div>
      <div className="Play">
        <div className="Play__prev" onClick={prev} />
        <div className="Play__currentDate">{moment(date).format('LL')}</div>
        <div className="Play__next" onClick={next} />
        <div className="Play__reset" onClick={reset} />
      </div>
    </div>
  );
};
